...
class MissionGenerator:
    ...
    def make_mission(
        ...
        return Mission(
            mission_id=mission_id or self._next_id(),
            source_id=source_id,
            day=day,
            phase=phase,
            mission_type=mtype,
    ...
    def make_ads_mission(
        ...
        return Mission(
            ...
            steps=[
                f"Откройте рекламный кабинет {platform_str.upper()}",
                "Создайте кампанию",
                f"Установите бюджет {budget}₽",
                "Запустите на 3 дня",
                "Проверьте результат",
            ],
    ...
    def make_sales_mission(
        ...
        return Mission(
            ...
            cta="Отправить",
            metric="2+ ответа",
            success_threshold="2+ ответа",
            warning_threshold="1 ответ",
            fail_threshold="0 ответов",
            if_success="Повторить завтра",
            if_fail="Сменить скрипт",
            xp_reward=30,
            avatar_id=avatar_id,
            pain_id=pain_id,
            offer_id=offer_id,
